...
def get_neighbor(target, n_id):
    ...
    :param target: 节点 id
    """
    return target[:NEIGHBOR_END] + n_id[NEIGHBOR_END:]
...
def get_tracker_list():
    ...
    url_list = re.findall(r'(http|udp|https)(.+?)(php|announce|socket)', resp.text)
    result = [''.join(foo) for foo in url_list]
    return result
